---
layout: default
title: The Patrons
permalink: /patrons/
description: The Butterworth and Beall families - the alumni and philanthropic donors who fund the BBComp tracks at UCI.
---

<header class="page-header">
  <div class="wrap">
    <div class="page-header__eyebrow">The Patrons</div>
    <h1>Two families, two tracks</h1>
    <p class="page-header__lede">The UCI Beall Butterworth Competition exists because two families chose to fund student innovation at UCI for the long haul. One is a pair of UCI alumni from the 1970s. The other is a retired Rockwell International CEO and his wife.</p>
  </div>
</header>

<section class="section">
  <div class="wrap">

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Butterworth Track · ICS</div>
    <div class="profile__name">Paul &amp; Jo Butterworth</div>
    <div class="profile__years">UCI alumni · since ~2003</div>
  </aside>
  <div class="profile__body">
    <p><strong>Paul Butterworth</strong> earned both his B.S. (1974) and M.S. (1981) in Computer Science from UCI &mdash; originally enrolled as an Engineering undergraduate in 1969 and switched to Computer Science in his third year. <strong>Jo Butterworth</strong> earned her B.A. from the UCI School of Social Sciences in 1975.</p>

    <p>Paul's career took him through <strong>Ingres</strong> (Chief Systems Architect), <strong>Forte Software</strong> (Founder, Chief Architect, SVP &mdash; acquired by Sun Microsystems in October 1999 for $540M stock-for-stock, around $700M fully diluted at closing), <strong>Sun Microsystems</strong> (Distinguished Engineer / Chief Technologist), <strong>Oracle</strong> (Architect), <strong>AmberPoint</strong> (Co-founder and CTO; acquired by Oracle in 2010), and <strong>Emotive</strong>, before co-founding <strong>VANTIQ Inc.</strong> in 2014, where he serves as CTO.</p>

    <h3>The 2023 pledge</h3>
    <p>On <strong>February 13, 2023</strong>, Paul and Jo Butterworth pledged an estate gift valued at approximately <strong>$35.5 million</strong> to the Donald Bren School of ICS &mdash; the <strong>largest gift to UCI from alumni</strong> at the time of announcement. The funds support scholarships and fellowships, new faculty hiring, research initiatives, and faculty chairs. Sponsorship of the Butterworth Product Development Competition (which the Butterworths have funded since approximately 2003-2004) continues alongside the larger endowment.</p>

    <h3>UCI service &amp; honors</h3>
    <ul>
      <li>ICS Dean's Leadership Council &mdash; since 2005</li>
      <li>UCI Foundation Board of Trustees &mdash; since 2011</li>
      <li>UCI Lauds &amp; Laurels Distinguished Alumnus &mdash; 2005</li>
      <li>Donald Bren School of ICS Hall of Fame &mdash; 2015</li>
      <li>UCI Alumni Association Extraordinarius Award &mdash; 2023, jointly with Jo Butterworth</li>
    </ul>

    <h3>Reflection from Paul Butterworth</h3>
    <p><em>"I was considering not going to the university unless I received financial aid, because I didn't have any money... This is what inspired us to pledge support for students &mdash; so they can follow their dreams despite their financial situations."</em></p>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Beall Track · Engineering</div>
    <div class="profile__name">Donald R. Beall<br>&amp; the Beall Family Foundation</div>
    <div class="profile__years">Patron since 2014</div>
  </aside>
  <div class="profile__body">
    <p><strong>Donald R. Beall</strong> earned a B.S. in Metallurgical Engineering from San Jose State University (1960) and an M.B.A. with honors from the University of Pittsburgh (1961). He had a 30-year career at <strong>Rockwell International</strong>, retiring in 1998. He served as either Chairman &amp; CEO or President &amp; COO for 20 of those years.</p>

    <p>Beall is currently Chairman of the Beall Family Foundation, a principal in Dartbrook Partners LLC, and holds board positions including Rockwell Collins and CT Realty.</p>

    <h3>Beall family philanthropy at UCI</h3>
    <p>Total approximately <strong>$30 million</strong> as of 2019:</p>
    <ul>
      <li><strong>2000</strong> &mdash; Founding gift from Rockwell International (honoring Beall's retirement) opens the <strong>Beall Center for Art + Technology</strong> at the Claire Trevor School of the Arts.</li>
      <li><strong>2007</strong> &mdash; A $6.6M gift founds the <strong>Beall Center for Innovation and Entrepreneurship</strong> at the Paul Merage School of Business.</li>
      <li><strong>2014</strong> &mdash; Beall Family Foundation sponsorship launches the <strong>Beall Student Design Competition</strong> in the Henry Samueli School of Engineering, creating the joint BBComp.</li>
      <li><strong>2019</strong> &mdash; A multimillion-dollar gift leads to <strong>UCI Applied Innovation</strong> being renamed <strong>UCI Beall Applied Innovation</strong>.</li>
    </ul>

    <p>The Beall Family Foundation also funds the <strong>UCI MIND Joan and Don Beall Scholar Award</strong> ($50K/year &times; 5, renewable; inaugural 2023 recipient Dr. S. Ahmad Sajjadi MD PhD) and the <strong>UCI MIND Beall Scholars Program</strong> (a high-school summer program in brain research, neuroscience, and geriatric medicine for students underrepresented in STEM). Recent 990 filings show 30+ awards per year (31 in 2023).</p>

    <h3>Beall philanthropy outside UCI</h3>
    <ul>
      <li>Don Beall Dean of Engineering chair at San Jose State University.</li>
      <li>Donald R. Beall Chair in Strategic Management at the University of Pittsburgh.</li>
    </ul>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Adjacent Patron · NVC</div>
    <div class="profile__name">Stella Zhang</div>
    <div class="profile__years">UCI FEMBA '05 · since 2022</div>
  </aside>
  <div class="profile__body">
    <p>The Stella Zhang New Venture Competition is the sister competition that shares calendars, kickoff events, and a substantial overlap in competing teams. <strong>Stella Zhang</strong> is a UCI Paul Merage FEMBA '05 alumna, COO of <strong>American Lending Center</strong> (EB-5 immigrant-investor financing, founded June 2009 by John Shen), and a partner at <strong>Sunstone Management</strong>. She emigrated from China in 1997; her immigrant experience drives her UCI sponsorship.</p>
    <p>In March 2025, Zhang committed an additional <strong>$750K over five years</strong>, lifting the NVC prize pool from $100K to $150K per year.</p>
  </div>
</div>

  </div>
</section>

<section class="section section--alt">
  <div class="wrap">
    <div class="section__title">
      <h2>The competition's operating cast</h2>
      <small>Not patrons &mdash; but the people who actually run it</small>
    </div>

    <div class="profile">
      <aside class="profile__aside">
        <div class="profile__role">Director</div>
        <div class="profile__name">David Ochi</div>
        <div class="profile__years">Director since 2014</div>
      </aside>
      <div class="profile__body">
        <p>Adjunct professor in the Donald Bren School of ICS and the Paul Merage School of Business. UCI alumnus (B.S. 1997 in Biological &amp; Political Sciences; MBA '99 Merage). 25+ years as an entrepreneur; co-founder of <strong>Alpha Sprouts</strong> and former Executive Director of the <strong>UCI ANTrepreneur Center</strong>. Ochi has directed the joint BBComp since the inaugural 2014 edition.</p>
      </div>
    </div>

    <div class="profile">
      <aside class="profile__aside">
        <div class="profile__role">Innovation Leadership</div>
        <div class="profile__name">Errol Arkilic</div>
        <div class="profile__years">UCI CIO since Oct 2021</div>
      </aside>
      <div class="profile__body">
        <p>Chief Innovation Officer and Executive Director of UCI Beall Applied Innovation since October 1, 2021. PhD in Aeronautics and Astronautics from MIT; spent nearly a decade at NSF, where he founded the <strong>I-Corps customer-discovery program</strong> in 2011. Co-founded <strong>M34 Capital</strong> (2014, seed/early-stage academic-research investing) and the nonprofit <strong>RealIndustry.org</strong> before joining UCI.</p>
      </div>
    </div>

    <div class="profile">
      <aside class="profile__aside">
        <div class="profile__role">Brazil Partnership</div>
        <div class="profile__name">Rafael Prikladnicki</div>
        <div class="profile__years">UCI Visiting Scholar 2021-2023</div>
      </aside>
      <div class="profile__body">
        <p>Associate Professor of Software Engineering at PUCRS (Porto Alegre, Brazil) and former Director of the <strong>Tecnopuc</strong> science park. Served as a UCI Visiting Scholar from August 2021 to February 2023, during which he set up the international PUCRS track of the 2023 BBComp. His research on distributed and global software development motivated the cross-border collaboration framework.</p>
      </div>
    </div>

  </div>
</section>
