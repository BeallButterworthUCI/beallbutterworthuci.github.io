---
layout: default
title: Faculty Mentors
permalink: /faculty/
description: UCI faculty members who have mentored multiple BBComp teams - Prof. Michelle Khine, Dr. Christine King, Prof. Ramesh Jain, and more.
---

<header class="page-header">
  <div class="wrap">
    <div class="page-header__eyebrow">Faculty Mentors</div>
    <h1>The professors behind the placements</h1>
    <p class="page-header__lede">A handful of UCI faculty members have mentored multiple BBComp teams across the years. The densest of these mentor pipelines is the Khine Lab, which has produced six BBComp competitors who became Khine Lab PhD alumni.</p>
  </div>
</header>

<section class="section">
  <div class="wrap">

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Biomedical Engineering</div>
    <div class="profile__name">Prof. Michelle Khine</div>
    <div class="profile__years">UCI BME · since 2009</div>
  </aside>
  <div class="profile__body">
    <p>Professor in the Henry Samueli School of Engineering, affiliated with the Beckman Laser Institute. Founding Director of <strong>Faculty Innovation</strong> at Samueli and founding Director of <strong>BioENGINE</strong>. UCI Associate Dean of Undergraduate Education since 2021.</p>

    <h3>Notable work</h3>
    <p>Khine's "Shrinky Dink microfluidics" technique &mdash; printing microfluidic patterns onto Shrinky Dink polymer sheets, then heating them in a toaster oven &mdash; underpins both <strong>Vena Vitals</strong> and <strong>Makani Science</strong>. Honors include the 2009 MIT Technology Review TR35 Award (Top 35 Innovators Under 35 in the world).</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2014 Beall 1st</strong> &mdash; Spero Diagnostics (sepsis diagnostic, co-mentored with Prof. Zoran Nenadic)</li>
      <li><strong>2014 Beall 2nd</strong> &mdash; A Hundred Tiny Hands (children's inventor toolboxes)</li>
    </ul>

    <h3>The Khine pipeline</h3>
    <p>At least <strong>six BBComp competitors</strong> went on to become Khine Lab PhD alumni:</p>
    <ul>
      <li>From 2014 A Hundred Tiny Hands: <strong>Sophia Lin</strong>, <strong>Eugene Lee</strong>, <strong>Jolie Nokes</strong></li>
      <li>From 2019 Qardiac: <strong>Joshua "Josh" Kim</strong>, <strong>Lancy Lin</strong>, <strong>En-Fan "Sophia" Chou</strong></li>
    </ul>
    <p>This is the densest faculty-to-PhD pipeline documented in the BBComp archive.</p>

    <h3>Khine spinoff companies</h3>
    <ul>
      <li><strong>Shrink Nanotechnologies</strong> (her first spinoff, from her UC Merced tenure)</li>
      <li><strong>Fluxion Biosciences</strong> (single-cell electroporation, from her PhD work)</li>
      <li><strong>Vena Vitals</strong> (Y Combinator S20; co-founded 2019 with 2019 Beall 1st team member Josh Kim and Ray Liu)</li>
      <li><strong>Makani Science</strong> (FDA 510(k) cleared April 2, 2025; $1.1M NIH NHLBI grant)</li>
      <li><strong>Novoheart</strong> (cardiac modeling; Scientific Advisory Board)</li>
    </ul>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Biomedical Engineering</div>
    <div class="profile__name">Dr. Christine King</div>
    <div class="profile__years">UCI BME · 2025 AIMBE Fellow</div>
  </aside>
  <div class="profile__body">
    <p>Associate Professor of Teaching in Biomedical Engineering at the Henry Samueli School of Engineering. BS and MS in Mechanical Engineering from Manhattan College; PhD in Biomedical Engineering from UCI (brain-computer interface systems for neurorehabilitation).</p>

    <h3>Honors</h3>
    <p>Inducted into the <strong>2025 Class of AIMBE Fellows</strong> (March 2025) for <em>"pioneering and impactful contributions in biomedical engineering education and engineering design for women's health, and leadership in these areas."</em> 2024 honorable mention for Dr. De Gallow Professor of the Year. Selected for the UCI Faculty Academy for Teaching Excellence (FATE).</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2024 Beall 1st</strong> &mdash; NeoMold (3D-printable customizable ear mold for neonates with congenital ear deformities)</li>
      <li><strong>2025 Beall 2nd</strong> &mdash; VeinGuard (non-compressive cervical collar; co-medical-advisor with Dr. Yama Akbari)</li>
      <li><strong>2025 Beall 3rd</strong> &mdash; REACH (Andrew Eck is an undergraduate researcher in her lab)</li>
    </ul>

    <p>King's lab is the documented faculty origin for <strong>three Beall placements across the 2024-2025 cycle</strong> &mdash; the densest two-year faculty-mentor BBComp run in this archive.</p>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Biomedical Engineering</div>
    <div class="profile__name">Prof. Zoran Nenadic</div>
    <div class="profile__years">UCI BME · 2025 AIMBE Fellow</div>
  </aside>
  <div class="profile__body">
    <p>Associate Professor, UCI Biomedical Engineering, Henry Samueli School of Engineering. Research focus: brain-computer interfaces and neural signal processing. Inducted into the 2025 Class of AIMBE Fellows alongside Christine King (March 2025).</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2014 Beall 1st</strong> &mdash; Spero Diagnostics (co-mentored with Prof. Michelle Khine)</li>
    </ul>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Computer Science</div>
    <div class="profile__name">Prof. Ramesh Jain</div>
    <div class="profile__years">First Bren Professor · 2005</div>
  </aside>
  <div class="profile__body">
    <p>Distinguished Professor Emeritus in the Department of Computer Science, Donald Bren School of ICS. Founding Director of the <strong>UCI Institute for Future Health</strong>. In 2005, named the <strong>first Bren Professor in Information and Computer Science</strong> &mdash; the named-chair role that anchors the school's faculty leadership. Sometimes called the "Father of Multimedia." Fellow of ACM, IEEE, AAAS, IAPR, AAAI, and SPIE.</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2012 Butterworth Finalist</strong> &mdash; Pictorria (web-based image-understanding service)</li>
      <li><strong>2013 Butterworth Finalist</strong> &mdash; Dentiverse (medical practice manager)</li>
    </ul>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Informatics</div>
    <div class="profile__name">Prof. Don Patterson</div>
    <div class="profile__years">UCI ICS</div>
  </aside>
  <div class="profile__body">
    <p>Professor in the Informatics Department, Donald Bren School of ICS. Research areas: ubiquitous computing, human-computer interaction, AI, context-aware computing, sensors / GPS / RFID / accelerometers, gesture recognition, cloud computing. Notable contributions to "collapse informatics" &mdash; the intersection of HCI with sustainability and ICT4D.</p>

    <h3>Honors</h3>
    <p>UBICOMP 10-Year Impact Award. AIJ 5-Year Impact Award. CHI 2012 Best Paper Award. 2014 Outstanding Professor Award for the Donald Bren School of ICS.</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2012 Butterworth Finalist</strong> &mdash; Rims (RFID Inventory Management System)</li>
      <li><strong>2012 Butterworth Finalist</strong> &mdash; Whobot (social shopping)</li>
    </ul>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Engineering · MAE</div>
    <div class="profile__name">Prof. Lawrence Kulinsky</div>
    <div class="profile__years">UCI MAE · BiNoM Lab</div>
  </aside>
  <div class="profile__body">
    <p>Director of the <strong>Kulinsky BiNoM (Bio-Nano-Micro-manufacturing) Lab</strong> in UCI's Department of Mechanical and Aerospace Engineering. Research focus: centrifugal microfluidic (CD) diagnostic platforms, microfluidic spinning, and electrokinetic cell manipulation.</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2024 Beall 3rd</strong> &mdash; PetPointDx (at-home pet fecal testing kits)</li>
    </ul>
  </div>
</div>

<div class="profile">
  <aside class="profile__aside">
    <div class="profile__role">Computer Science</div>
    <div class="profile__name">Prof. Ian Harris</div>
    <div class="profile__years">UCI ICS</div>
  </aside>
  <div class="profile__body">
    <p>Associate Professor, Computer Science, Donald Bren School of ICS. Research areas: embedded systems, hardware verification, cybersecurity, IoT.</p>

    <h3>BBComp mentees</h3>
    <ul>
      <li><strong>2012 Butterworth Finalist</strong> &mdash; SmartHome (mobile-controlled home appliances)</li>
    </ul>
  </div>
</div>

  </div>
</section>
